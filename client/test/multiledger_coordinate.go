...
func TestMultiLedgerCoordinate(
	ctx context.Context,
	t *testing.T,
	...
	// Wait until Bob's watcher processed the coordinate.
	time.Sleep(100 * time.Millisecond) //nolint:mnd // The 100ms is a guess on how long the watcher needs to catch up.
	...
	coordSig, err := channel.Sign(charlie.WalletAccount[bID1], reqCoord.Tx.State, bID1)
	require.NoError(err)
	err = charlie.Adjudicator1.Coordinate(ctx, reqCoord, nil, []wallet.Sig{coordSig})
	require.NoError(err)
